...
a[f].matches),c[P]){for(d=++f;e>d&&!w.relative[a[d].type];d++);return K(1<f&&u(h),1<f&&E(a.slice(0,f-1).concat({value:" "===a[f-2].type?"*":""})).replace(X,"$1"),c,d>f&&F(a.slice(f,d)),e>d&&F(a=a.slice(d)),e>d&&E(a))}h.push(c)}return u(h)}function x(a,b){var d=0,A=0<b.length,f=0<a.length,g=function(e,g,k,h,l){var Q,n,q=[],ua=0,z="0",m=e&&[],E=null!=l,Ca=aa,p=e||f&&w.find.TAG("*",l&&g.parentNode||g),u=U+=null==Ca?1:Math.random()||0.1;for(E&&(aa=g!==D&&g,t=d);null!=(l=p[z]);z++){if(f&&l){for(Q=0;n=
a[Q++];)if(n(l,g,k)){h.push(l);break}E&&(U=u,t=++d)}A&&((l=!n&&l)&&ua--,e&&m.push(l))}if(ua+=z,A&&z!==ua){for(Q=0;n=b[Q++];)n(m,q,g,k);if(e){if(0<ua)for(;z--;)m[z]||q[z]||(q[z]=ca.call(h));q=O(q)}ba.apply(h,q);E&&!e&&0<q.length&&1<ua+b.length&&c.uniqueSort(h)}return E&&(U=u,aa=Ca),m};return A?e(g):g}var B,r,t,w,Y,s,v,aa,y,H,D,R,I,C,la,G,J,P="sizzle"+-new Date,da=a.document,U=0,N=0,L=d(),ma=d(),pa=d(),V=!1,M=function(a,b){return a===b?(V=!0,0):0},ja=typeof b,S=-2147483648,qa={}.hasOwnProperty,Z=[],
...
0;d=a.parentNode;var A=b.parentNode,f=[a],g=[b];if(a===b)return V=!0,0;if(!d||!A)return a===c?-1:b===c?1:d?-1:A?1:y?W.call(y,a)-W.call(y,b):0;if(d===A)return h(a,b);for(d=a;d=d.parentNode;)f.unshift(d);for(d=b;d=d.parentNode;)g.unshift(d);for(;f[e]===g[e];)e++;return e?h(f[e],g[e]):f[e]===da?-1:g[e]===da?1:0},c):D};c.matches=function(a,b){return c(a,null,null,b)};c.matchesSelector=function(a,b){if((a.ownerDocument||a)!==D&&H(a),b=b.replace(ka,"='$1']"),r.matchesSelector&&I&&!(la&&la.test(b)||C&&C.test(b)))try{var d=
...
e?function(a){return!!a.parentNode}:function(b,c,k){var h,l,Q,na,n;c=A!==f?"nextSibling":"previousSibling";var q=b.parentNode,ua=g&&b.nodeName.toLowerCase();k=!k&&!g;if(q){if(A){for(;c;){for(l=b;l=l[c];)if(g?l.nodeName.toLowerCase()===ua:1===l.nodeType)return!1;n=c="only"===a&&!n&&"nextSibling"}return!0}if(n=[f?q.firstChild:q.lastChild],f&&k)for(k=q[P]||(q[P]={}),h=k[a]||[],na=h[0]===U&&h[1],Q=h[0]===U&&h[2],l=na&&q.childNodes[na];l=++na&&l&&l[c]||(Q=na=0)||n.pop();){if(1===l.nodeType&&++Q&&l===b){k[a]=
...
b,c){for(c=0>c?c+b:c;b>++c;)a.push(c);return a})}};w.pseudos.nth=w.pseudos.eq;for(B in{radio:!0,checkbox:!0,file:!0,password:!0,image:!0})w.pseudos[B]=l(B);for(B in{submit:!0,reset:!0})w.pseudos[B]=n(B);z.prototype=w.filters=w.pseudos;w.setFilters=new z;v=c.compile=function(a,b){var c,d=[],e=[],A=pa[a+" "];if(!A){b||(b=m(a));for(c=b.length;c--;)A=F(b[c]),A[P]?d.push(A):e.push(A);A=pa(a,x(e,d))}return A};r.sortStable=P.split("").sort(M).join("")===P;r.detectDuplicates=V;H();r.sortDetached=f(function(a){return 1&
...
Space.prototype.clear=function(a){if(this.isEmpty())return this;this._clear();this.trigger("clear");a&&this._load(a);this.trigger("change");return this};Space.prototype.clone=function(){return new Space(this.toString())};Space.prototype.concat=function(a){"string"===typeof a&&(a=new Space(a));var d=this;a.each(function(a,b){d.append(a,b)});return this};Space.prototype.create=function(a,d){this._setTuple(a,d);this.trigger("create",a,d);this.trigger("change");return this};
...
"expressfs.downloadDirectory",b).done(c).fail(c)},exists:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.exists",c,function(a){a.match(/ does NOT exist/)?d(!1):d(!0)})},mkdir:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.mkdir",c,function(a){d&&d()})},readdir:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.readdir",c).done(function(a){d&&d(JSON.parse(a))}).fail(d)},readFile:function(a,
...
a.remove()});var f=this;d.val(this.get("value"));d.focus();d.on("change",function(){f.set("value",new Space(d.val()));f.save();f.render();Insight.insightOn&&Insight.base.insight();a.remove()})};Insight.Record.prototype.element=function(){return $("#"+this.id)};Insight.dragIt=function(a){a.draggable({stop:function(){var a=$(this).attr("id"),c=$(this),a=Insight.base.get(a);a.set("meta y",c.position().top);a.set("meta x",c.position().left);a.save()},start:function(){$(".InsightEditor").remove()}})};
Insight.Record.prototype.render=function(){$("#"+this.id).remove();var a=$('<i class="icon-location-arrow"></i>'),d=$('<div class="InsightRecord"></div>');a.css("font-size","14px");a.css("-webkit-transform","rotate(80deg)");a.css("transform","rotate(80deg)");d.append(a);a="";this.get("value label")?a=this.get("value label"):this.get("value name")?a=this.get("value name"):this.get("value title")?a=this.get("value title"):this.get("value Title")?a=this.get("value Title"):this.get("value Name")?a=this.get("value Name"):
this.get("value")&&(a=this.get("value").getByIndex(0));a&&d.append("<label>"+a+"</label>");d.attr("id",this.id);var a=this.get("meta x"),c=this.get("meta y");d.css({top:c+"px",left:a+"px"});$(".InsightPlane").append(d);Insight.dragIt(d);return this};Insight.Record.prototype.save=function(a){expressfs.writeFile("nudgepad/insight/"+this.database+"/"+this.id+".space",this.toString(),a)};Insight.Database=function(a){this.name=a;this.settings=new Space;return this};Insight.Database.prototype=new Space;
Insight.Database.prototype.edit=function(){var a=this;TextPrompt.open("Edit Settings",this.settings.toString(),"settings.space",function(d){a.settings.reload(d);a.save();a.render()})};Insight.Database.prototype.getPath=function(a){return"nudgepad/insight/"+(a?a:this.name)};
Insight.Database.prototype.insight=function(){Insight.insightOn=!0;view=this.settings;var a=view.get("y");if(!a)return!1;$(".InsightRecord").addClass("InsightRecordAnimate");var d=$(".InsightPlane").height()-20,c,b;this.each(function(d,e,h){d=parseFloat(e.get("value "+a));"undefined"===typeof c&&(c=d);"undefined"===typeof b&&(b=d);c>d&&(c=d);b<d&&(b=d)});var e=b-c;console.log(b);this.each(function(b,g,h){b=g.element();g=g.get("value "+a);g=Math.round((parseFloat(g)-c)/e*d);console.log(g);g=d-g;b.css("top",
g)});setTimeout("$('.InsightRecord').removeClass('InsightRecordAnimate')",1E3);$("#InsightYMax").html(b).show();$("#InsightYLabel").html(a).show();$("#InsightYMin").html(c).show();a=view.get("x");if(!a)return!1;console.log("moving");$(".InsightRecord").addClass("InsightRecordAnimate");d=$(".InsightPlane").width()-100;b=c=void 0;this.each(function(d,e,h){d=parseFloat(e.get("value "+a));"undefined"===typeof c&&(c=d);"undefined"===typeof b&&(b=d);c>d&&(c=d);b<d&&(b=d)});e=b-c;console.log(b);this.each(function(b,
g,h){b=g.element();g=g.get("value "+a);g=Math.round((parseFloat(g)-c)/e*d);console.log(g);b.css("left",g)});setTimeout("$('.InsightRecord').removeClass('InsightRecordAnimate')",1E3);$("#InsightXMax").html(b).show();$("#InsightXLabel").html(a).show();$("#InsightXMin").html(c).show()};
...
Insight.Database.prototype.save=function(){expressfs.writeFile(this.getPath()+"/settings.space",this.settings.toString())};Insight.Database.prototype.trash=function(a){expressfs.unlink("nudgepad/insight/"+this.name+"/"+a+".space",function(){$("#"+a).fadeOut("fast",function(){$(this).remove()})})};Insight.drop={};
...
Pages.on("ready",function(){$("#PagesStage").attr("src","");$("#PagesStage").on("load",function(){store.get("PagesFilename")&&Pages.editor.open(store.get("PagesFilename"))});expressfs.readdir("",function(a){var d="";a.forEach(function(a,b){d+=a+"\n"});Pages.editor.updateMenu(d)});Pages.dirWatcher=socketfs.watch("",function(a){Pages.editor.updateMenu(a.content)})});
