...
Space.prototype.set=function(a,d,c){a=a.toString();Space.isXPath(a)?this._setByXPath(a,d):this.has(a)?this._setTuple(a,d,this.indexOf(a),!0):this._setTuple(a,d,c);this.trigger("set",a,d,c);this.trigger("change");return this};
...
Insight.Record.prototype.render=function(){$("#"+this.id).remove();var a=$('<i class="icon-location-arrow"></i>'),d=$('<div class="InsightRecord"></div>');a.css("font-size","14px");a.css("-webkit-transform","rotate(80deg)");a.css("transform","rotate(80deg)");d.append(a);a=Insight.base.settings.get("label")||"name";(a=this.get("value "+a))&&d.append("<label>"+a+"</label>");d.attr("id",this.id);var a=this.get("meta x"),c=this.get("meta y");d.css({top:c+"px",left:a+"px"});$(".InsightPlane").append(d);
Insight.dragIt(d);return this};Insight.Record.prototype.save=function(a){expressfs.writeFile("nudgepad/insight/"+this.database+"/"+this.id+".space",this.toString(),a)};Insight.Database=function(a){this.name=a;this.settings=new Space;return this};Insight.Database.prototype=new Space;Insight.Database.prototype.create=function(a){expressfs.mkdir(this.getPath(),a)};Insight.Database.prototype.getPath=function(a){return"nudgepad/insight/"+(a?a:this.name)};
Insight.Database.prototype.insight=function(){this.updateX();this.updateY()};Insight.Database.prototype.open=function(){var a=this;expressfs.downloadDirectory(this.getPath(),"space",function(d){a.reload(d);a.get("settings.space")&&(a.settings.reload(a.get("settings.space")),a.delete("settings.space"));a.each(function(a,b,d){a=a.replace(/\.space/,"");b=new Insight.Record(a,this.name,b);this.update(d,a,b)});a.render();$("#InsightDatabase").html(a.name);store.set("InsightDatabase",a.name)})};
Insight.Database.prototype.rename=function(a){expressfs.rename(this.getPath(),this.getPath(a),function(){Insight.menu.open(a)})};Insight.Database.prototype.render=function(){$(".InsightPlane").html("");this.each(function(a,d,c){d.render()});$(".InsightPlane").show()};Insight.Database.prototype.saveSettings=function(){expressfs.writeFile("nudgepad/insight/"+this.name+"/settings.space",this.settings.toString())};
...
Insight.Database.prototype.updateY=function(){var a=this.settings.get("y");if(!a)return!1;$(".InsightRecord").addClass("InsightRecordAnimate");var d=$(".InsightPlane").height()-20,c,b;this.each(function(d,e,h){d=parseFloat(e.get("value "+a));"undefined"===typeof c&&(c=d);"undefined"===typeof b&&(b=d);c>d&&(c=d);b<d&&(b=d)});var e=b-c;console.log(b);this.each(function(b,g,h){b=g.element();g=g.get("value "+a);g=Math.round((parseFloat(g)-c)/e*d);console.log(g);g=d-g;b.css("top",g)});setTimeout("$('.InsightRecord').removeClass('InsightRecordAnimate')",
1E3);$("#InsightYMax").html(b).show();$("#InsightYLabel").html(a).show();$("#InsightYMin").html(c).show()};
Insight.Database.prototype.updateX=function(){var a=this.settings.get("x");if(!a)return!1;console.log("moving");$(".InsightRecord").addClass("InsightRecordAnimate");var d=$(".InsightPlane").width()-100,c,b;this.each(function(d,e,h){d=parseFloat(e.get("value "+a));"undefined"===typeof c&&(c=d);"undefined"===typeof b&&(b=d);c>d&&(c=d);b<d&&(b=d)});var e=b-c;console.log(b);this.each(function(b,g,h){b=g.element();g=g.get("value "+a);g=Math.round((parseFloat(g)-c)/e*d);console.log(g);b.css("left",g)});
setTimeout("$('.InsightRecord').removeClass('InsightRecordAnimate')",1E3);$("#InsightXMax").html(b).show();$("#InsightXLabel").html(a).show();$("#InsightXMin").html(c).show()};Insight.drop={};Insight.drop.traverseFileTree=function(a,d){d=d||"";a.isFile?a.file(function(a){console.log("File:",d+a.name);Insight.drop.processFile(d,a)}):a.isDirectory&&a.createReader().readEntries(function(c){for(var b=0;b<c.length;b++)Insight.drop.traverseFileTree(c[b],d+a.name+"/")})};
