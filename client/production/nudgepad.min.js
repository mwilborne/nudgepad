...
useAce:function(){return TextPrompt.ace},value:function(a){return a?TextPrompt.useAce()?TextPrompt.editor.setValue(a):$("#TextPromptTextarea").val(a):TextPrompt.useAce()?TextPrompt.editor.getValue():$("#TextPromptTextarea").val()}},Backup=new Tool("Backup");Backup.on("ready",function(){$("#BackupDownload").attr("href","/nudgepad.backup/"+document.location.host+".zip")});var Blog=new Tool("Blog");Blog.set("posts",new Space);Blog.active={};Blog.active.filename=null;
