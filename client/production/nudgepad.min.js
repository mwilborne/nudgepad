...
"expressfs.rename",b,function(a){c&&c()})},rmdir:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.rmdir",c,function(a){d&&d()})},unlink:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.unlink",c,function(a){d&&d()})},writeFile:function(a,d,c){var b={};b.path=expressfs.rootPath+a;b.content=d;$.post(expressfs.prefix+"expressfs.writeFile",b,function(a){c&&c()})},writeFileAndOpen:function(a,d,c,b){if(platform&&"Firefox"===platform.name)return expressfs.writeFile(a,
d,function(){window.open(a,c);b&&b()}),!0;var e=$('<form target="'+c+'" method="post" action="'+expressfs.prefix+'expressfs.writeFile"></form>'),f=$('<input name="path">');f.val(expressfs.rootPath+a);var g=$('<textarea name="content"></textarea>');g.val(d);d=$('<input name="redirect">');d.val(a+"?"+(new Date).getTime());e.append(f);e.append(g);e.append(d);e.submit()},writeFileBase64:function(a,d,c){var b={};b.path=expressfs.rootPath+a;b.content=d;b.encoding="base64";$.post(expressfs.prefix+"expressfs.writeFile",
b,c)}},socketfs={rootPath:""};socketfs.files=new Space;socketfs.Watcher=function(a,d){this.filename=a;this.fn=d;return this};socketfs.Watcher.prototype.unwatch=function(a){socketfs.unwatch(this.filename,this.fn,a)};socketfs.Tail=function(a,d){this.filename=a;this.fn=d;return this};socketfs.Tail.prototype.untail=function(a){socketfs.untail(this.filename,this.fn,a)};socketfs.connect=function(){socketfs.main(io.connect("/"))};socketfs.inspect=function(a){socketfs.socket.emit("inspect","get",a)};
...
Fiddle.save=function(){var a=new Space;"A B C D E F".split(/ /g).forEach(function(d){var c=$("#"+d).val();c&&a.set("values "+d,c);(c=$(".FiddleCode[name="+d+"]").val())&&a.set("code "+d,c)});store.set("Fiddle",a.toString())};var Files=new Tool("Files");Files.set("path","");Files.on("ready",function(){Files.refresh();Files.updateHidden();Files.watcher=socketfs.watch("",function(a){Files.set("files",new Space(a.content));Files.renderExplorer()},function(){})});
...
Home.on("ready",function(){Home.renderMenu();$(window).on("resize",Home.heights)});Home.on("close",function(){$(window).off("resize",Home.heights)});var Insight=new Tool("Insight");Insight.permalink=function(a){return a?a.toLowerCase().replace(/[^a-z0-9- _\.]/gi,"").replace(/ /g,"-"):""};Insight.menu={};Insight.database;Insight.view=new Space;Insight.menu.create=function(){var a=prompt("Enter a name for your new database","untitled");if(!a)return!1;a=Insight.permalink(a);(new Insight.Database(a)).create(function(){Insight.menu.open(a)})};
Insight.menu.open=function(a){Insight.database=a;Insight.base=new Insight.Database(a);Insight.base.open()};Insight.menu.openPrompt=function(){var a=prompt("Open a db","untitled");if(!a)return!1;Insight.menu.open(a)};Insight.menu.editView=function(){TextPrompt.open("Edit View",Insight.view.toString(),"view.space",function(a){Insight.view.reload(a);Insight.view.save();Insight.base.render()})};
...
Pages.editor.duplicate=function(){var a=prompt("Enter a name for the file",Pages.filename);if(!a)return!0;expressfs.writeFile(a,Pages.page.toHtml());Pages.pages.push(a);Pages.editor.open(a);Pages.refresh()};Pages.editor.loadFiles=function(){expressfs.downloadDirectory("","html",function(a){a=new Space(a);Pages.pages=a.getKeys();Pages.refresh()})};
Pages.on("ready",function(){$("#PagesStage").attr("src","");$("#PagesStage").on("load",function(){store.get("PagesFilename")&&Pages.editor.open(store.get("PagesFilename"))});Pages.refresh()});Pages.on("close",function(){Events.shortcut.shortcuts={};Events.shortcut.disableShortcutsIfInputHasFocus=!0;Pages.watcher&&Pages.watcher.unwatch()});Pages.on("open",Pages.editor.loadFiles);
