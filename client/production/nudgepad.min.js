...
a;e.extension=d;c&&(e.content=c);$.post(expressfs.prefix+"expressfs.createUntitled",e,b)},dirStats:function(a,d){var c={};c.path=expressfs.rootPath+a;c.path.match(/\/$/)||(c.path+="/");$.post(expressfs.prefix+"expressfs.dirStats",c).done(d).fail(d)},downloadDirectory:function(a,d,c){var b={};b.path=expressfs.rootPath+a;b.extension=d;b.path.match(/\/$/)||(b.path+="/");$.post(expressfs.prefix+"expressfs.downloadDirectory",b).done(c).fail(c)},exists:function(a,d){var c={};c.path=expressfs.rootPath+a;
$.post(expressfs.prefix+"expressfs.exists",c,function(a){a.match(/ does NOT exist/)?d(!1):d(!0)})},mkdir:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.mkdir",c,function(a){d&&d()})},readdir:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.readdir",c).done(function(a){d&&d(JSON.parse(a))}).fail(d)},readFile:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.readFile",c).done(d).fail(d)},
rename:function(a,d,c){var b={};b.oldPath=expressfs.rootPath+a;b.newPath=expressfs.rootPath+d;if(!d)return console.log("No name provided");$.post(expressfs.prefix+"expressfs.rename",b,function(a){c&&c()})},rmdir:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.rmdir",c,function(a){d&&d()})},unlink:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.unlink",c,function(a){d&&d()})},writeFile:function(a,d,c){var b={};b.path=expressfs.rootPath+
a;b.content=d;$.post(expressfs.prefix+"expressfs.writeFile",b,function(a){c&&c()})},writeFileAndOpen:function(a,d,c,b){if(platform&&"Firefox"===platform.name)return expressfs.writeFile(a,d,function(){window.open(a,c);b&&b()}),!0;var e=$('<form target="'+c+'" method="post" action="'+expressfs.prefix+'expressfs.writeFile"></form>'),f=$('<input name="path">');f.val(expressfs.rootPath+a);var g=$('<textarea name="content"></textarea>');g.val(d);d=$('<input name="redirect">');d.val(a+"?"+(new Date).getTime());
e.append(f);e.append(g);e.append(d);e.submit()},writeFileBase64:function(a,d,c){var b={};b.path=expressfs.rootPath+a;b.content=d;b.encoding="base64";$.post(expressfs.prefix+"expressfs.writeFile",b,c)}},socketfs={rootPath:""};socketfs.files=new Space;socketfs.Watcher=function(a,d){this.filename=a;this.fn=d;return this};socketfs.Watcher.prototype.unwatch=function(a){socketfs.unwatch(this.filename,this.fn,a)};socketfs.Tail=function(a,d){this.filename=a;this.fn=d;return this};
socketfs.Tail.prototype.untail=function(a){socketfs.untail(this.filename,this.fn,a)};socketfs.connect=function(){socketfs.main(io.connect("/"))};socketfs.inspect=function(a){socketfs.socket.emit("inspect","get",a)};
