...
for(var p=Array(16),r=Array(16),x=0;16>x;x++)p[x]=s[x]^909522486,r[x]=s[x]^1549556828;c=e(p.concat(g(c)),512+c.length*d);c=e(r.concat(c),768);return f(c)},sha256config:function(a){d=parseInt(a)||8}});a.fn.sha256=function(b){a.sha256config(b);b=h(a(this).val());b=a.sha256(b);a.sha256config(8);return b}})(jQuery);function Space(a){this._tuples=[];this.events={};this._load(a);return this}Space.version="0.5.1";Space.arrayDelete=function(a,d){return a.slice(0,d).concat(a.slice(d+1))};Space.isXPath=function(a){return a.match(/ /)};
...
"expressfs.rename",b,function(a){c()})},rmdir:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.rmdir",c,function(a){d()})},unlink:function(a,d){var c={};c.path=expressfs.rootPath+a;$.post(expressfs.prefix+"expressfs.unlink",c,function(a){d()})},writeFile:function(a,d,c){var b={};b.path=expressfs.rootPath+a;b.content=d;$.post(expressfs.prefix+"expressfs.writeFile",b,function(a){c&&c()})},writeFileAndOpen:function(a,d,c,b){c=$('<form target="'+c+'" method="post" action="'+
expressfs.prefix+'expressfs.writeFile"></form>');b=$('<input name="path">');b.val(expressfs.rootPath+a);var e=$('<textarea name="content"></textarea>');e.val(d);d=$('<input name="redirect">');d.val(a+"?"+(new Date).getTime());c.append(b);c.append(e);c.append(d);c.submit()},writeFileBase64:function(a,d,c){var b={};b.path=expressfs.rootPath+a;b.content=d;b.encoding="base64";$.post(expressfs.prefix+"expressfs.writeFile",b,c)}},socketfs={rootPath:""};socketfs._watching=new Space;socketfs.connect=function(){socketfs.main(io.connect("/"))};
socketfs.main=function(a){socketfs.socket=a;socketfs.socket.on("change",function(a){if(!socketfs._watching.get(a.filename))return!1;socketfs._watching.get(a.filename).each(function(c,b){b(a)})})};socketfs.unwatch=function(a,d){a=socketfs.rootPath+a;if(!socketfs._watching.get(a))return!1;socketfs._watching.get(a).each(function(c,b){b===d&&socketfs._watching.get(a).delete(c)})};
socketfs.watch=function(a,d){a=socketfs.rootPath+a;socketfs._watching.get(a)||socketfs._watching.set(a,new Space);socketfs._watching.get(a).push(d);socketfs.socket.emit("watch",a,function(a){})};socketfs.watchers=function(){socketfs.socket.emit("watchers","nothing",function(a){console.log("response:");console.log(a)})};socketfs.watching=function(){return socketfs._watching};function Tool(a){this.clear();this.events={};this.set("name",a);this._open=!1;Tool.tools.push(a);return this}Tool.tools=[];
Tool.opened={};Tool.openedAt=null;Tool.openTool=null;Tool.prototype=new Space;Tool.prototype.close=function(a){if(!this._open)return!1;this.logTime();this.trigger("close");this._open=!1;Tool.openTool=null;$("#OpenTool").html("");a&&window[a].open()};Tool.prototype.isOpen=function(){return this._open};
...
Blog.downloadPosts=function(){expressfs.downloadDirectory("nudgepad/posts/","space",function(a){a=new Space(a);a.sort(function(a,c){return c[0]>a[0]});a.each(function(a,c){Blog.set("posts "+a,new Blog.Post(a,c))});Blog.trigger("posts")})};Blog.getList=function(){var a="<ul>\n";Blog.get("posts").each(function(d,c){a+=' <li><a href="'+c.get("permalink")+'">'+c.get("title")+"</a></li>\n"});a+="</ul>";TextPrompt.open("List",a,"list.html")};
...
Pages.editor.loadFiles=function(){Pages.editor.pages=new Space;expressfs.downloadDirectory("","html",function(a){(new Space(a)).each(function(a,c){Pages.editor.pages.set(a,new Scraps.Page($.htmlToScraps(c)));a===Pages.editor.filename&&Pages.editor.open(Pages.editor.filename);Pages.refresh()})})};Pages.on("ready",function(){Pages.editor.open(Pages.editor.filename);Pages.refresh()});Pages.on("open",Pages.editor.loadFiles);
