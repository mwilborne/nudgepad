...
expressfs.prefix+'expressfs.writeFile"></form>');b=$('<input name="path">');b.val(expressfs.rootPath+a);var e=$('<textarea name="content"></textarea>');e.val(d);d=$('<input name="redirect">');d.val(a+"?"+(new Date).getTime());c.append(b);c.append(e);c.append(d);c.submit()},writeFileBase64:function(a,d,c){var b={};b.path=expressfs.rootPath+a;b.content=d;b.encoding="base64";$.post(expressfs.prefix+"expressfs.writeFile",b,c)}},socketfs={rootPath:""};socketfs.watchers=new Space;
socketfs.Watcher=function(a,d){this.filename=a;this.fn=d;return this};socketfs.Watcher.prototype.unwatch=function(a){socketfs.unwatch(this.filename,this.fn,a)};socketfs.connect=function(){socketfs.main(io.connect("/"))};socketfs.main=function(a){socketfs.socket=a;socketfs.socket.on("change",function(a){if(!socketfs.watchers.get(a.filename))return!1;socketfs.watchers.get(a.filename).each(function(c,b){b(a)})})};
socketfs.unwatch=function(a,d,c){a=socketfs.rootPath+a;if(!socketfs.watchers.get(a))return!1;d?socketfs.watchers.get(a).each(function(b,c){c===d&&socketfs.watchers.get(a).delete(b)}):socketfs.watchers.delete(a);socketfs.watchers.get(a)?c&&c():socketfs.socket.emit("unwatch",a,function(a){c&&c(a)})};socketfs.unwatchGlobal=function(a){socketfs.watchers=new Space;socketfs.socket.emit("unwatchGlobal","get",a)};
socketfs.watch=function(a,d,c){a=socketfs.rootPath+a;socketfs.watchers.get(a)||socketfs.watchers.set(a,new Space);socketfs.watchers.get(a).push(d);c||(c=function(){});socketfs.socket.emit("watch",a,c);return new socketfs.Watcher(a,d)};socketfs.getAllWatchers=function(a){socketfs.socket.emit("watchers","get",a)};function Tool(a){this.clear();this.events={};this.set("name",a);this._open=!1;Tool.tools.push(a);return this}Tool.tools=[];Tool.opened={};Tool.openedAt=null;Tool.openTool=null;
Tool.prototype=new Space;Tool.prototype.close=function(a){if(!this._open)return!1;this.logTime();this.trigger("close");this._open=!1;Tool.openTool=null;$("#OpenTool").html("");a&&window[a].open()};Tool.prototype.isOpen=function(){return this._open};Tool.prototype.logTime=function(){var a=(new Date).getTime()-Tool.openedAt;expressfs.appendFile("nudgepad/time/"+Cookie.email+".space",Socket.socket.sessionid+" "+this.get("name")+" "+a+"\n",function(){Tool.openedAt=(new Date).getTime()})};
