...
Grid.prototype.addDynamicPoints=function(){var a=this;if(200<$("#DesignerStageBody #body").children(".DesignerScrap:not(.DesignerSelection)").length)return!0;$("#DesignerStageBody #body").children(".DesignerScrap:not(.DesignerSelection)").each(function(){var b=$(this).scrap();b&&a.addPoints(b.selector())})};
...
$.fn.insertAt=function(a,b){var d=this.children().size();0>a&&(a=Math.max(0,d+1+a));this.append(b);a<d&&this.children().eq(a).before(this.children().last());return this};$.fn.deselect=function(a){var b=$(this).attr("id");$(this).removeClass("DesignerSelection");$("."+b+"Handle").remove();a||Designer.trigger("selection");return $(this)};
...
Designer.MoveHandle.selectTopScrap=function(){var a=$("#DesignerStageBody").offset().left,b=$("#DesignerStageBody").offset().top,a=$.topDiv(".DesignerScrap:visible",Designer.Mouse.down.pageX-a,Designer.Mouse.down.pageY-b+Designer.stage.scrollTop());if(!a||a.hasClass("DesignerSelection")||a.scrap().get("locked"))return!0;Designer.stage.selection.clear();a.selectMe();return!0};
Designer.MoveHandle.slide=function(a,b){var d=$(this).owner();d.scrap();var c=Designer.MoveHandle.dimensions,e=Designer.stage.scrollTop()-Designer.MoveHandle.scrollTop,f={y:0,x:0};b.shiftKey||(f=Designer.grid.getDelta([{x:c.left+Designer.Mouse.xChange,y:c.top+Designer.Mouse.yChange+e},{x:c.right+Designer.Mouse.xChange,y:c.bottom+Designer.Mouse.yChange+e},{x:c.center+Designer.Mouse.xChange,y:c.middle+Designer.Mouse.yChange+e}]));var k=Designer.Mouse.yChange+e+f.y,g=Designer.Mouse.xChange+f.x;$(".DesignerSelection").each(function(){$(this).scrap().move(g-
...
Designer.onkeydown=function(a){if(0!=$("input:focus, div:focus, textarea:focus, a:focus").length||(a.ctrlKey||a.metaKey||a.shiftKey)||(48>a.keyCode&&32!=a.keyCode||90<a.keyCode)||!$(".DesignerSelection").length||$(".DesignerSelection").is("input")||$(".DesignerSelection").is("textarea"))return!0;$(".DesignerSelection").scrap().edit()};
...
Scrap.disableLinks=function(a){a.preventDefault();return!1};Scrap.selectOnTap=function(){$(this).is(":focus")||$(":focus").blur();if(Designer.Mouse.down&&Designer.Mouse.down.altKey&&!$(this).hasClass("DesignerSelection")&&$(".DesignerSelection").length)return Designer.stage.selection.nest($(this).attr("path")),!1;(!Designer.Mouse.down||!Designer.Mouse.down.shiftKey)&&Designer.stage.selection.clear();$(this).selectMe();return!1};
...
Designer.stage.selection.saved=[];Designer.stage.selection.capture=function(){Designer.stage.selection.captured=Designer.stage.selection.toSpace()};Designer.stage.selection.clear=function(){if(!$(".DesignerSelection").length)return!0;$(".DesignerSelection").each(function(){$(this).deselect(!0)});Designer.trigger("selection")};Designer.stage.selection.css=function(a){Designer.stage.selection.cssPreview(a);Designer.stage.commit();$(".handle").trigger("update")};
Designer.stage.selection.cssPreview=function(a){if(!a)return!1;a=new Space(a);$(".DesignerSelection").each(function(){var b=$(this).scrap().get("style");b||($(this).scrap().set("style",new Space),b=$(this).scrap().get("style"));b.patch(a);$(this).css(a.values)})};Designer.stage.selection.delete=function(){$(".DesignerSelection").each(function(){var a=$(this).scrap();$(this).deselect(!0).remove();a&&Designer.page.delete(a.getPath())})};
Designer.stage.selection.duplicate=function(){$(".DesignerSelection").each(function(){$(this).duplicate()});Designer.stage.commit()};Designer.stage.selection.editSource=function(){Designer.stage.selection.capture();Designer.stage.selection.save();TextPrompt.open("Enter code...",Designer.stage.selection.captured.toString(),Designer.stage.selection.modify)};Designer.stage.selection.exists=function(){return $(".DesignerSelection").length};
...
Designer.stage.selection.move=function(a,b){if(!$(".DesignerSelection").length)return!1;$(".DesignerSelection").each(function(){$(this).scrap().move(a,b)});var d=$($(".DesignerSelection")[0]),c="X "+parseFloat(d.css("left"))+"<br>Y "+parseFloat(d.css("top"));$("#DesignerDimensions").css({left:10+d.offset().left+d.outerWidth(),top:-10+d.offset().top+Math.round(d.outerHeight()/2)}).html(c);Popup.open("#DesignerDimensions");$(".handle").trigger("update");Designer.stage.commit()};
...
Designer.stage.selection.patch=function(a){"string"===typeof a&&(a=new Space(a));$(".DesignerSelection").each(function(){var b=$(this).scrap();$(this).deselect();b.patch(a);$(this).replaceWith(b.toHtml(Scrap.devFilter));b.element().selectMe()});Designer.stage.commit()};Designer.stage.selection.restore=function(){for(var a in Designer.stage.selection.saved){var b=Designer.stage.selection.saved[a];$(b).length&&$(b).selectMe(!0)}Designer.trigger("selection")};
Designer.stage.selection.save=function(){Designer.stage.selection.saved=[];$(".DesignerSelection").each(function(){Designer.stage.selection.saved.push($(this).scrap().selector())})};Designer.stage.selection.toSpace=function(){var a=new Space;$(".DesignerSelection").each(function(){var b=$(this).scrap();a.set(b.getPath(),new Space(b.toString()))});return a};
Designer.broadcastSelection=function(a){var b=a||"",d="";$(".DesignerSelection").each(function(){$(this).scrap()&&(b+=d+$(this).scrap().selector(),d=",")});b+="{box-shadow: 0 0 4px "+Screen.get("color")+";cursor: not-allowed;}";Screen.set("selection",b)};
...
(e=$(".DesignerSelection").dimensions(),d=Math.round($("#DesignerStageBody").width()/2-e.width/2),c=Math.round(Designer.stage.scrollTop()+$(window).height()/2-e.height/2));(d||c)&&$(".DesignerSelection").each(function(){$(this).scrap().move(d,c)});$(".handle").trigger("update");if(b){var g=$(".DesignerSelection").attr("id");Events.slide.target=$("#moveHandle"+g);$("#moveHandle"+g).triggerHandler("mousedown");$("#moveHandle"+g).triggerHandler("slidestart");$(".DesignerSelection").each(function(){var a=
$(this),b=a.clone(),c=a.css("opacity"),d=$(this).scrap();a.css("opacity","0.01");b.attr("id","nudgepad_move_ghost").removeClass("DesignerScrap DesignerSelection");b.on("mousedown",function(){a.remove()});b.css("font-family",$("#DesignerStageBody").css("font-family"));d.values.style&&b.css(d.values.style.values);b.css({"z-index":"600",position:"fixed",top:a.offset().top,left:a.offset().left});$("#moveHandle"+g).on("slide",function(){b.css({top:a.offset().top,left:a.offset().left})});$("#moveHandle"+
g).on("slideend",function(){a.css("opacity",c);b.remove()});$("body").append(b)})}else Designer.stage.commit();return k};Designer.stage.isBehind=function(){return Designer.stage.version<Designer.stage.timeline.keys.length};
...
Designer.stage.scrollTop=function(){return $("#DesignerStage").scrollTop()};Designer.stage.selectAll=function(){$(".DesignerScrap").each(function(){$(this).selectMe(!0)});Designer.trigger("selection")};
