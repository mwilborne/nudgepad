...
a="Clone Files Pages Server Team Templates Labs".split(/ /g);$(".navbar-collapse").children().first().prepend('<li class="dropdown"><a class="cursor dropdown-toggle" data-toggle="dropdown">Tools <b class="caret"></b></a><ul class="dropdown-menu" id="MoreTools"></ul></li>');$("#MoreTools").html("");a.forEach(function(a,c){var b=ToolInfo.get(a.toLowerCase()).get("icon")||"picture";$("#MoreTools").append('<li><a class="cursor" onclick="Launcher.open(\''+a+'\')"><i class="icon-'+b+'" style="width: 20px;display: inline-block;"></i> '+
a+"</a></li>")});$(".navbar-right .tool-help").on("click",function(){var a=Tool.openTool.get("name"),c=ToolInfo.get(a.toLowerCase()),b=$('<div id="ToolInfoBox"></div>');b.append('<p style="text-align: center; color: #888;"><i>About this Tool</i></p>');b.append("<h1>"+a+"</h1>");b.append("<h3>"+c.get("description")+"</h3>");b.append("<h4>By "+c.get("author")+"</h4>");b.append('<p>Email comments, feature requests and bug reports to <a target="_blank" href="mailto:'+c.get("email")+'">'+c.get("email")+
"</a></p>");PreviewBox.open(b)});$(window).scrollTop(0);Tool.openTool=this;this._open=!0;this.trigger("ready");mixpanel.track("I opened the "+this.get("name")+" tool");Screen.set("tool",this.get("name"))};Tool.prototype.restart=function(){this.close();this.open()};Tool.prototype.test=function(){var a=this.get("name");$.get("/nudgepad/tools/"+a+"/tests.html",function(d){$("body").append(d);$("body").append('<script type="text/javascript" src="/nudgepad/tools/'+a+'/tests.js">\x3c/script>')})};
...
Clone.on("ready",function(){$("#CloneDomain").val("copyof"+document.location.host);$("#CloneServer").val(Project.get("hostname"))});Clone.on("once",function(){expressfs.readFile("nudgepad/sharecode.txt",function(a){a?Clone.code=a:Clone.install()})});
...
Labs.renderMenu=function(){$("#OpenTool #LabsContainer").html("");var a="Backup Blog Clone Crawler Files Flickr Git Pages Redirect Server Share Sketch Stats Team Templates Time".split(/ /g),d="rgb(26,134,214) rgb(36,65,112) rgb(231,189,44) rgb(171,193,199) rgb(224,54,52) rgb(71,41,54)".split(" "),c='<div class="row">',b;for(b in a){var e=window[a[b]];0<b&&0===b%3&&(c+='</div><div class="row">');var e=e.get("name"),f=ToolInfo.get(e.toLowerCase()),c=c+Labs.toButton(e,f.get("description"),d[b?b%d.length:
0],f.get("icon")||"picture")}0!==(b+1)%3&&(c+="</div>");$("#OpenTool #LabsContainer").append(c);Labs.heights()};Labs.heights=function(){var a=0;$(".jumbotron").css("height","auto");$(".jumbotron").each(function(){$(this).height()>a&&(a=$(this).height())}).height(a)};
