...
Grid.prototype.addDynamicPoints = function () {
  ...
  if ($('#DesignerStageBody #body').children('.DesignerScrap:not(.' + Designer.stage.selection.className + ')').length > 200)
    return true
  $('#DesignerStageBody #body').children('.DesignerScrap:not(.' + Designer.stage.selection.className + ')').each(function(){
...
Grid.prototype.addFixedPoints = function () {
  ...
  // verticals
  var start = $('#DesignerStageBody').position().left
  var end = $('#DesignerStageBody').position().left + $('#DesignerStageBody').outerWidth()
  this.horizontalSpacing = parseFloat(this.horizontalSpacing)
  for (var i = start; i <= end; i = i + this.horizontalSpacing) {
    this.addPoint(i, 0, '#DesignerStageBody')  
  }
  
  start = $('#DesignerStageBody').position().top
  end = $('#DesignerStageBody').position().top + $('#DesignerStageBody').outerHeight()
  this.verticalSpacing = parseFloat(this.verticalSpacing)
  for (var i = start; i <= end; i = i + this.verticalSpacing) {
    this.addPoint(0, i, '#DesignerStageBody')  
  }
...
//    this.points.clear()
  ...
  if (this.snapToContainer) {
    ...
    // left
//    this.addPoint(0, $('#DesignerStageBody').position().top, '#DesignerStageBody')
    // right
    this.addPoint($('#DesignerStageBody').outerWidth(), 0, '#DesignerStageBody')
    // center
    this.addPoint(Math.round($('#DesignerStageBody').outerWidth()/2), 0, '#DesignerStageBody')
    // top
    this.addPoint(0, 0, '#DesignerStageBody')
    // bottom
    this.addPoint(0, $('#DesignerStageBody').height(), '#DesignerStageBody')
    // middle
    this.addPoint(0, Math.round($(window).height()/2), '#DesignerStageBody')
...
Grid.prototype.draw = function () {
  ...
  for (var i in this.points) {
    ...
    if (point.selector !== '#DesignerStageBody')
      continue
...
//  console.log(point2.selector)
  ...
  // Dont draw lines based on objects
  if (point2.selector !== '#DesignerStageBody')
...
Grid.prototype.drawSnaplineCanvas = function () {
  ...
  this.width = $("#DesignerStageBody").width()
  this.height = $("#DesignerStageBody").height()
